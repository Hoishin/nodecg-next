...
test(
	...
	testEffect(
		Effect.gen(function* () {
			...
			yield* inMemoryStateStorage.set(
				"test-effect-decode-fail",
				"count",
				"not a number",
			);

			const result = yield* Effect.either(state.count.getValue());
			expect(result._tag).toBe("Left");
		}),
	),
);
...
test(
	...
	testEffect(
		Effect.gen(function* () {
			...
			expect(yield* state.when.getValue()).toEqual(new Date(0));

			const newDate = new Date("2026-05-14T00:00:00.000Z");
			yield* state.when.set(newDate);

			const wire = yield* inMemoryStateStorage.get("test-codec", "when");
			expect(wire).toBe("2026-05-14T00:00:00.000Z");
			expect(yield* state.when.getValue()).toEqual(newDate);
		}),
	),
);
